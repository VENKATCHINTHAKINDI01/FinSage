  ...
  return (
    <div className="min-h-screen w-full grid grid-cols-1 lg:grid-cols-2 bg-slate-50 dark:bg-slate-950 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="hidden lg:flex flex-col justify-between bg-gradient-to-br from-navy/90 via-deep-ocean/95 to-navy-deep/95 text-white p-12 relative overflow-hidden backdrop-blur-md border-r border-white/5 z-10">
        ...
        <div className="absolute -right-24 -top-24 w-96 h-96 rounded-full bg-gradient-to-br from-cosmic-cyan/20 to-cosmic-violet/10 blur-3xl animate-float" />
        <div className="absolute -left-16 bottom-0 w-72 h-72 rounded-full bg-gradient-to-tr from-cosmic-emerald/15 to-transparent blur-3xl animate-float-slow" />
        <div className="absolute right-10 bottom-20 w-48 h-48 rounded-full bg-gradient-to-tr from-cosmic-violet/10 to-transparent blur-3xl animate-float" style={{ animationDelay: '-6s' }} />

        <div className="relative flex items-center gap-2.5 animate-gravity-left">
          <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-cosmic-cyan to-cosmic-violet flex items-center justify-center shadow-lg shadow-cosmic-cyan/20">
        ...
        <div className="relative space-y-2 animate-gravity-up" style={{ animationDelay: '0.15s' }}>
          ...
          <div className="space-y-4 pt-2 stagger">
            {HIGHLIGHTS.map(({ icon: Icon, text }, i) => (
              <div key={i} className="flex items-center gap-3 group">
                <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center shrink-0 transition-all duration-300 group-hover:scale-110 group-hover:bg-cosmic-cyan/20 aurora-border">
                  <Icon size={15} />
                </div>
      ...
      {/* Form panel — Premium Glassmorphic container */}
      <div className="relative z-10 flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-[430px] p-8 sm:p-10 rounded-[24px] border border-white/40 dark:border-slate-800/40 bg-white/70 dark:bg-slate-900/65 backdrop-blur-xl shadow-xl shadow-slate-100/50 dark:shadow-cosmic-cyan/5 animate-gravity-up aurora-border">
          <div className="lg:hidden flex items-center gap-2.5 mb-8">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cosmic-cyan to-cosmic-violet flex items-center justify-center">
              <IndianRupee size={16} strokeWidth={2.5} className="text-white" />
            </div>
            <span className="font-display font-semibold text-[16px] tracking-tight text-ink dark:text-white">FinSage AI</span>
          </div>

          <h1 className="font-display font-semibold text-[24px] text-ink dark:text-white mb-1.5">{title}</h1>
          <p className="text-[13.5px] text-ink-soft dark:text-slate-400 mb-8">{subtitle}</p>
